  ...
  constructor(gl, shaderType, sourceFileName) {
    ...
    if (source) {
      const marked = source.replace(
        /[^\x00-\x7F]/g,
        (match) => `\x1b[46m${match}\x1b[30m`,
      );
      if (marked !== source) {
        console.error(`\x1b[46mNON-ASCII:\x1b[30m\n${marked}`);
        throw new Error(
          `Shader source file ${sourceFileName} has non-ASCII characters.`,
        );
      }
      ...
      throw new Error(
        `Shader ${sourceFileName} not found. Check spelling, and whether the essl file is embedded into the html file.`,
      );
    }
